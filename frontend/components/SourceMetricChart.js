...
export function SourceMetricChart({ points, label }) {
  ...
  return (
    <div className="h-56 w-full overflow-hidden rounded-xl border border-slate-800 bg-panel p-3">
      <ResponsiveContainer width="100%" height="100%">
        <LineChart data={data} margin={{ top: 8, right: 8, left: -14, bottom: 6 }}>
          ...
          <Line type="monotone" dataKey="metricValue" stroke="#f59e0b" strokeWidth={2.5} dot={false} isAnimationActive={false} />
        </LineChart>
      </ResponsiveContainer>
    </div>
  );
